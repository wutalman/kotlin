...
public class CandidateResolver {
    ...
    private static <D extends CallableDescriptor> ResolutionStatus checkReceiver(
        ...
        if (!TypeUtils.dependsOnTypeParameters(receiverParameter.getType(), candidateDescriptor.getTypeParameters())) {
            ...
            autoCasted = AutoCastUtils.recordAutoCastIfNecessary(receiverArgument, receiverParameter.getType(), context);
        }
        ...
        if (!safeAccess && !receiverParameter.getType().isNullable() && receiverArgument.getType().isNullable()) {
            ...
            if (!autoCasted && isExplicitReceiver) {
