    ...
    public ClassDescriptor getClassDescriptor(@NotNull JetClassOrObject classOrObject) {
        ...
        if (descriptor == null) {
            if (scopeDescriptor != null) {
                // Force resolution for all classifiers with given name because getClassifier() didn't triggered resolution for
                // desired declaration. Search was probably stopped when first matching descriptor had been found.

                resolutionScope.getClassifiers(name);
                descriptor = getBindingContext().get(BindingContext.DECLARATION_TO_DESCRIPTOR, classOrObject);
            }
