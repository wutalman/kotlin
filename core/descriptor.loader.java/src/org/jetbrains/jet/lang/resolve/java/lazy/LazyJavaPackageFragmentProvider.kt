...
public open class LazyJavaPackageFragmentProvider(
    ...
    override fun getSubPackagesOf(fqName: FqName) = getPackageFragment(fqName)
                                                                        ?.let {it.getMemberScope().getSubPackages()}
                                                                        ?: listOf<FqName>()
    override fun getClassNamesInPackage(packageName: FqName) = getPackageFragment(packageName)
                                                                    ?.let {it.getMemberScope().getAllClassNames()}
                                                                    ?: listOf<Name>()
